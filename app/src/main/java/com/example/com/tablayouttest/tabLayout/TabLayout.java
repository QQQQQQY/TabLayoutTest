package com.example.com.tablayouttest.tabLayout;
...
/**
 * @Author: qq.yang.
 * @Date: 2018/6/19 0019 11:21.
 * @Description: 修改系统源码实现 下划线和文字长度一样.
 */
@ViewPager.DecorView
public class TabLayout extends HorizontalScrollView {
     ...
     * 设置下划线与标题一样长.
     */
    private final boolean mTabUnderWidthEqualTitle;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabUnderWidthEqualTitle = a.getBoolean(android.support.design.R.styleable.TabLayout_tabUnderLineWidthEqualTitle, false);
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            int underLineOffset = getUnderLineOffset(this, mSelectedPosition);

            final View selectedTitle = getChildAt(mSelectedPosition);
            int left, right;

            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                left = selectedTitle.getLeft() + underLineOffset / 2;
                right = selectedTitle.getRight() - underLineOffset / 2;

                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    // Draw the selection partway between the tabs
                    View nextTitle = getChildAt(mSelectedPosition + 1);
                    left = (int) (mSelectionOffset * nextTitle.getLeft() + underLineOffset / 2 +
                            (1.0f - mSelectionOffset) * left);
                    right = (int) (mSelectionOffset * nextTitle.getRight() - underLineOffset / 2 +
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            int underLineOffset = getUnderLineOffset(this, position);

            final int targetLeft = targetView.getLeft() + underLineOffset / 2;
            final int targetRight = targetView.getRight() - underLineOffset / 2;
            final int startLeft;
    ...
    private int getUnderLineOffset(ViewGroup parent, int mSelectedPosition) {
        if (mTabUnderWidthEqualTitle) {
            if (mSelectedPosition != parent.getChildCount() - 1) { // 设置 tab view 的 下划线 和 title 文字一样长度.
                final LinearLayout selectTabView = (LinearLayout) parent.getChildAt(mSelectedPosition);
                TextView tvTitle = (TextView) selectTabView.getChildAt(1);
                return selectTabView.getWidth() - tvTitle.getWidth();
            }
        }
        return 0;
    }
